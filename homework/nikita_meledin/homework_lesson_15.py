...
# 2. Добавляем книги и сразу указываем, кто их взял (через taken_by_student_id)
books = [
...
books_with_student = [(title, author, student_id) for title, author in books]
cursor.executemany(
    "INSERT INTO books (title, author, taken_by_student_id) VALUES (%s, %s, %s)",
    books_with_student
)
...
# 8. Проверяем книги, которые взял студент (где taken_by_student_id = student_id)
cursor.execute("""
    SELECT title, author
    FROM books
...
cursor.execute("""
    SELECT 
        ...
        GROUP_CONCAT(DISTINCT CONCAT(b.title, ' (', b.author, ')') SEPARATOR '; ') as books,
        GROUP_CONCAT(DISTINCT CONCAT(subj.name, ': ', m.mark) SEPARATOR '; ') as marks_list
    FROM students s
